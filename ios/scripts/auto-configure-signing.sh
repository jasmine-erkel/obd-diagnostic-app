...
if [[ "$GIT_EMAIL" == *"jasmine"* ]] || [[ "$GIT_EMAIL" == *"jasmineerkel"* ]]; then
    ...
    TEAM_ID="MC3KGY63CP"
    BUNDLE_ID="com.joshuaerkel.obddiagnosticapp"
else
...
# Update the project file with the detected team and bundle ID
if [ -f "$PROJECT_FILE" ]; then
    # Create a temporary file for sed operations
    TEMP_FILE="${PROJECT_FILE}.tmp"

    # Replace DEVELOPMENT_TEAM and PRODUCT_BUNDLE_IDENTIFIER
    sed -e "s/DEVELOPMENT_TEAM = \"[^\"]*\";/DEVELOPMENT_TEAM = \"$TEAM_ID\";/g" \
        -e "s/PRODUCT_BUNDLE_IDENTIFIER = com\.[^;]*;/PRODUCT_BUNDLE_IDENTIFIER = $BUNDLE_ID;/g" \
        "$PROJECT_FILE" > "$TEMP_FILE"

    # Only update if sed succeeded
    if [ $? -eq 0 ]; then
        mv "$TEMP_FILE" "$PROJECT_FILE"
        echo "✅ Auto-configured signing for $GIT_EMAIL"
        echo "   Team: $TEAM_ID"
